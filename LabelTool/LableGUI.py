...
class MainGUI(tk.Frame):
    ...
    def __init__(self, root):
        ...
        self.x = 0  # ??画布上是这么获取x，y坐标的
        self.y = 0  # 存储坐标
        self.x_list = []
        self.y_list = []
        self.clicked = False
        self.default_values = []  # 标签默认值
    ...
    def initComponent(self, root):
        ...
        lable = tk.Label(self.frame_right, font='Times 9 bold', text="图片显示", fg='#4876FF', anchor='nw')
    ...
    def initButton(self):
        ...
        self.set_work_dir = ttk.Button(self.frm_control, text='导入文件', command=self.set_work_dir)
        ...
        self.next_image.grid(row=1, column=6, padx=5, sticky=tk.NSEW)

        self.pre_image = ttk.Button(self.frm_control, text='上一张', state=tk.DISABLED, command=self.get_pre_image)
        self.pre_image.grid(row=1, column=5, padx=5, sticky=tk.NSEW)

        self.revoke = ttk.Button(self.frm_control, text='撤销标注', state=tk.DISABLED, command=self.remove_canvas_lable_all)
        self.revoke.grid(row=1, column=4, padx=5, sticky=tk.NSEW)

        self.label_rect = ttk.Button(self.frm_control, text='创建矩形', state=tk.DISABLED, command=self.create_label_rect)
        self.label_rect.grid(row=1, column=2, padx=5, sticky=tk.NSEW)

        self.label_scat = ttk.Button(self.frm_control, text='标签', state=tk.DISABLED, command=self.create_label_scat)
        self.label_scat.grid(row=1, column=7, padx=5, sticky=tk.NSEW)

        self.save_lable = ttk.Button(self.frm_control, text='保存标注', state=tk.DISABLED, command=self.save_lable_file)
        self.save_lable.grid(row=1, column=3, padx=5, sticky=tk.NSEW)

        lable1 = tk.Label(self.frm_control, font='Times 9 bold', text="", fg='#4876FF', anchor='nw')
        lable1.grid(row=2, column=0, sticky=tk.NSEW, padx=6, pady=6)
    ...
    def mouse_on_canvas(self, event):
        ...
        if self.lable_Type == 1 and self.work_dir != '' and self.clicked:
            ...
            # 显示坐标位置
            self.canvas.create_rectangle((self.x, self.y), (event.x, event.y), width=1, outline='red',
    ...
    def left_mouse_click(self, event):
        self.canvas.create_oval((event.x - 3, event.y - 3, event.x + 3, event.y + 3), fill='red', tags='oval')

        if self.lable_Type == 1:
            if self.clicked:
                self.clicked = False
                if len(self.image_label) <= 1:
                    # 设置默认标签
                    r = simpledialog.askstring('标签', '', initialvalue=self.image_label)
                    # r标签名称
                    if self.image_index >= 0 and r != None:
                        ...
                        self.lable_save_file.set_lable_info(r, lable_name='name')
                        self.lable_save_file.set_position_rect('bndbox', xmin=self.x,
                                                               ymin=self.y, xmax=event.x,
                                                               ymax=event.y)
                else:
                    r = simpledialog.askstring('标签', '', initialvalue=self.image_label[-1])
                    if self.image_index >= 0 and r != None:
                        if r not in self.image_label:
                            self.insert_label_tree(self.lable_tree, r)  # 添加标签数
                            self.image_label.append(r)
            ...
            if not self.clicked:
                ...
                if len(self.scatter_point) > 0 and self.scatter_point[0][0] < event.x + 5 and self.scatter_point[0][
                    0] > event.x - 5 and self.scatter_point[0][1] < event.y + 5 and self.scatter_point[0][
                    1] > event.y - 5:
                    self.clicked = False
                    r = simpledialog.askstring('标签', '请输入标签', initialvalue='')
                    if self.image_index >= 0 and r != None:
